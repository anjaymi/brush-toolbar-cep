...
export const DebugPanel: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleGetOptions = async () => {
    try {
      ...
      // 显式提取 Shape Dynamics 关键路径以供检查
      const extractedShapeDynamics = {
        _raw_szVr: opts?.["$szVr"],
        _raw_angle: opts?.["angleDynamics"], 
        _raw_round: opts?.["roundnessDynamics"],
        
        parsed_sizeJitter: opts?.["$szVr"]?.jitter?._value,
        parsed_minDiameter: opts?.minimumDiameter?._value,
        parsed_angleJitter: opts?.angleDynamics?.jitter?._value,
        parsed_roundJitter: opts?.roundnessDynamics?.jitter?._value,
        parsed_minRoundness: opts?.minimumRoundness?._value
      };

      const debugData = {
        _EXTRACTED_SHAPE_DYNAMICS: extractedShapeDynamics,
